...
def handsChangeDirection(currentLeftHandDistance , currentRightHandDistance , results):
  ...
  leftIndexFingerPip = 0
  leftMiddleFingerPip = 0
  leftRingIndexFingerPip = 0
  leftPinkyPip = 0
  ...
  rightIndexFingerPip = 0
  rightMiddleFingerPip = 0
  rightRingIndexFingerPip = 0
  rightPinkyPip = 0
  ...
  for hand_index, hand_info in enumerate(results.multi_handedness):
        ...
        if (hand_label == "Left"):
          leftIndexFingerPip = hand_landmarks.landmark[6].y
          leftMiddleFingerPip = hand_landmarks.landmark[10].y
          leftRingIndexFingerPip = hand_landmarks.landmark[14].y
          leftPinkyPip = hand_landmarks.landmark[18].y
          ...
          rightIndexFingerPip = hand_landmarks.landmark[6].y
          rightMiddleFingerPip = hand_landmarks.landmark[10].y
          rightRingIndexFingerPip = hand_landmarks.landmark[14].y
          rightPinkyPip = hand_landmarks.landmark[18].y
  ...
  if not handsDirection:
    if ((currentLeftHandDistance < -(leftHandDistanceLevel / 8)) and (leftIndexFingerPip > leftIndexFingerTip) \
      and (leftMiddleFingerPip > leftMiddleFingerTip) and  (leftRingIndexFingerPip > leftRingIndexFingerTip) \
        and (leftPinkyPip > leftPinkyTip)):
          isLHandDown = True
    else:
      isLHandDown = False
              
    if ((currentRightHandDistance < -(rightHandDistanceLevel / 8)) and (rightIndexFingerPip > rightIndexFingerTip) \
      and (rightMiddleFingerPip > rightMiddleFingerTip) and  (rightRingIndexFingerPip > rightRingIndexFingerTip) \
        and (rightPinkyPip > rightPinkyTip)):
          isRHandDown = True
    else:
      isRHandDown = False
  ...
  # If your hands down
  if  handsDirection:
    if ((currentLeftHandDistance > (leftHandDistanceLevel / 2)) and (leftIndexFingerPip < leftIndexFingerTip) \
      and (leftMiddleFingerPip < leftMiddleFingerTip) and  (leftRingIndexFingerPip < leftRingIndexFingerTip) \
        and (leftPinkyPip < leftPinkyTip)):
          isLHandUp = True
    else:
    ...
    if ((currentRightHandDistance > (rightHandDistanceLevel / 2)) and (rightIndexFingerPip < rightIndexFingerTip) \
      and (rightMiddleFingerPip < rightMiddleFingerTip) and  (rightRingIndexFingerPip < rightRingIndexFingerTip) \
        and (rightPinkyPip < rightPinkyTip)):
          isRHandUp = True
    else:
            ...
            and (hand_landmarks.landmark[4].y < 1 and hand_landmarks.landmark[4].y > 0) \
              and (hand_landmarks.landmark[4].x < 1 and hand_landmarks.landmark[4].x > 0):
                ...
                if distanceWristMiddleFinglerMCP > 0.15 and  hand_label == "Right" and not isRightHandFixating \
                  and (rightIndexFingerPip < rightIndexFingerTip) \
                    and (rightMiddleFingerPip < rightMiddleFingerTip) and  (rightRingIndexFingerPip < rightRingIndexFingerTip) \
                      and (rightPinkyPip < rightPinkyTip):
                ...
                if distanceWristMiddleFinglerMCP > 0.15 and  hand_label == "Left" and not isLeftHandFixating\
                  and (leftIndexFingerPip < leftIndexFingerTip) \
                    and (leftMiddleFingerPip < leftMiddleFingerTip) and  (leftRingIndexFingerPip < leftRingIndexFingerTip) \
                      and (leftPinkyPip < leftPinkyTip):
